...
-- ----------------------------
INSERT INTO `exercises` VALUES ('1', '下列描述中正确的是', '算法是指对解题方案的准确而完整的描述', '算法的有穷性是指算法程序的长度是有限的', '算法是否有效与拥有的情报无关', '算法的基本特征有可行性、确定性和有穷性', 'A', '第1章_程序设计基本概念', '2', '无解析');
INSERT INTO `exercises` VALUES ('2', '下列数据结构中属于非线性结构的是', '队列', '线性表', '二叉树', '栈', 'C', '第1章_程序设计基本概念', '2', '无解析');
INSERT INTO `exercises` VALUES ('3', '在C语言中，合法的长整型常数是', '0.123456', '4962717', '9L', '5.321e2', 'C', '第1章_程序设计基本概念', '3', '无解析');
INSERT INTO `exercises` VALUES ('4', '执行语句X=(a=10，b=a--.后，x、a、b的值依次为', '10,10,9', '10,9,9', '9,10,9', '10,9,10', 'D', '第1章_程序设计基本概念', '4', '无解析');
INSERT INTO `exercises` VALUES ('5', '下面概念中，不属于面向对象方法的是', '对象', '继承', '类', '过程调用', 'D', '第1章_程序设计基本概念', '5', '无解析');
INSERT INTO `exercises` VALUES ('6', '下列选项中属于软件生命周期开发阶段任务的是', '可行性研究', '需求分析', '详细设计', '软件维护', 'C', '第1章_程序设计基本概念', '4', '无解析');
INSERT INTO `exercises` VALUES ('7', '软件设计中衡量模块独立性的度量标准是', '抽象和信息隐蔽', '局部化和封装化', '内聚性和耦合性', '激活机制和控制方法', 'C', '第1章_程序设计基本概念', '3', '无解析');
INSERT INTO `exercises` VALUES ('8', '下列不属于过程设计工具中图形工的是', 'N-S', 'PAD', '程序流程图', 'DFD', 'D', '第1章_程序设计基本概念', '4', '无解析');
INSERT INTO `exercises` VALUES ('9', '下列叙述中错误的是', '数据库管理系统是故据库的核心', '数据库系统由数据库、数据库管理系统、数据库管理员三部分组成', '数据共享最好的是数据库系统阶段', '数据库中的数据独立于应用程序而不依赖于应用程序', 'B', '第1章_程序设计基本概念', '5', '无解析');
INSERT INTO `exercises` VALUES ('10', '在下列选项中，哪个不是-', '无穷性', '可行性', '确定性', '有穷性', 'A', '第1章_程序设计基本概念', '5', '无解析');
INSERT INTO `exercises` VALUES ('11', '程序流程图中带有前头的线段表示的是', '图九美系', '教据漆', '控制疏', '调用关系', 'C', '2017年9月计算机二级考试《C语言》临考猜想卷', '1', '在数据流图中，用标有名字的前头表示故据流。右程序流程图中，用标有名字的箭头表示控\r\n制流。所以选择C');
INSERT INTO `exercises` VALUES ('12', '结构化程序设计的基本原则不包括', '多态性', '自頂向下', '模块化', '逐步求精', 'A', '2017年9月计算机二级考试《C语言》临考猜想卷', '2', '结构化程序设计的思想包括。自顶向下、逐步求精、模块化、限制使用goto语句，所以选择\r\nA\r\n');
INSERT INTO `exercises` VALUES ('13', '教件设计中模块划分应遵酒的准则是', '低内聚低耦合', '高内聚低耦合', '低内聚高耦合', '高内聚高耦合', 'B', '2017年9月计算机二级考试《C语言》临考猜想卷', '4', '软件设计中模块划分应遵语的准贝是高内聚低偶台、模块大小规模适当、模块的依赖关系适\r\n当等。模块的划分应遵语一定的要求，以保证模块划分合理，并进一步保证以此为依据开发出的软件系统可\r\n靠性强，县于理解和维护。模块之间的耦合应尽可能的低，模块的內聚度应尽可能的高');
INSERT INTO `exercises` VALUES ('14', '在软件开发中，需家分析阶段产生的主要文档是', '可行性分析报告', '软件需求规格说明书', '概要设计说明书', '集成测试计划', 'B', '2017年9月计算机二级考试《C语言》临考猜想卷', '3', '解析1A 增误，可行性分析阶码产生可行性分析报告。C)借误概要设计说明书是总体设计阶段产\r\n生的文档。D)错误，集成商试计划是在概要设计阶段编写的文档。B)正确，需求规格说明书是后续工作如设\r\nt、编码等需要的重要参考文档。');
INSERT INTO `exercises` VALUES ('15', '算法的有穷性是指', '算法程序的运行时间是有限的', '算法程序所处理的数据量是有限的', '算法程序的长度是有限的', '算法只能被有限的用户使用', 'A', '2017年9月计算机二级考试《C语言》临考猜想卷', '5', '算法原贝上能够情确地运行.而且人们用笔和纸做有限次运算后即可完成。有方性是指算法\r\n程序的运行时间是有限的');
INSERT INTO `exercises` VALUES ('16', '对长度为r的线住表排序，在最坏情况下，比较次数不是a(n一D/2的排序方法是', '快速排序', '冒泡排序', '直接插入排序', '堆排序', 'D', '2017年9月计算机二级考试《C语言》临考猜想卷', '3', '除了堆排序算法的比较次故是\r\nO(eleson)，其他的都是n(n一D/2.');
INSERT INTO `exercises` VALUES ('17', '下列关于栈的前述正确的是', '栈按“先进先出“组织数据', '栈按”先进后出“组织教据', '只能在栈底插入数据', '不能到际教据', 'B', '2017年9月计算机二级考试《C语言》临考猜想卷', '2', '栈是按失进出的原则组乡数据的数据的板和田际都在核债进行操作、\r\n笔记');
INSERT INTO `exercises` VALUES ('18', '在数据库设计中，将E一图转换成关系故据模型的过程属于', '需求分析阶段', '概念设计阶段', '逻辑设计阶段', '物理设计阶段', 'C', '2017年9月计算机二级考试《C语言》临考猜想卷', '2', 'E一R图转换成关系横型数据则是把图形分析出来的联系反映子数据库中，即设计出表，所以\r\n属于逻辑设计阶段。');
INSERT INTO `exercises` VALUES ('19', '有表示4 性选课的三张表，学生S( 学号，姓名，性別，年齡，身份证号)，课程C(课号，课名)，选课SC(学\r\n好，课号，成绩》，则表SC的关键字(健或码)为', '课号，成績', '学号，咸绩', '学号课号', '学号，姓名成绩', 'C', '2017年9月计算机二级考试《C语言》临考猜想卷', '3', '学号是学生表S的王键，课号是课程表C的主键，所选课表$的关键字就应该是与前两表\r\n能够直接联系且能唯一定义的学号和评号，所以选择C');
INSERT INTO `exercises` VALUES ('20', '以下额述中错误的是', 'C语言中的每条可扶行语句和非执行语句最终都将被转换成二进制的机器指令', 'c程序经过编详、连接步黎之后才能形成一个真正可执行的二进制机器指令文件', '用C语言编写的程序称为源程序，它以ASCI代码形式存放在一个文本文件中', 'C语言源程序经编译后生成后握为ob的目标程序', 'B', '2017年9月计算机二级考试《C语言》临考猜想卷', '3', 'C语言中的非执行语均不会被编评，不会生成二选制的机器指令，所以入)错误。由C语宫构\r\n成的指令予列称C源程序，C源程序经过C语言编译程序编评之后生成一个后援为。ORJ的二进制文件(称为目\r\n标文件) 最后委由“连接程序“把此。OBJ文件与C语言提供的各种库函数连接起来生成一个后提为，X的可\r\n执行文件。');
INSERT INTO `exercises` VALUES ('21', '以下选项中，合法的一组C语言效值常量是', '12.0Xa23 4 5e0', '028.5e-3-0xt', '，177 4o1.5 Oabc', '0x8A 10,000 3.e5', 'A', '2015年9月计算机二级考试《C语言》真题及解析', '4', 'C语言中八选制整型常量的开头是故字0 十六进制整型常量的开头数字是OX。C语言中的实\r\n型常量有两种表示形式，小数形式，小数形式表示的实型常量必须要有小致点指数元式，以”。“或下“后\r\n跟一个整故来表示以10为底数的某故，且规定字母。或之前必须要有数字，且。或后面的指数必页为整\r\n故。B)选项中028错误，八进制中没有数字8。C)选项中41.5中6后面的指数不是整数。D)选项中3.e5小数\r\n点后面缺少数字。');
INSERT INTO `exercises` VALUES ('22', '以下选项中不台法的标识符是', '&a', 'FOR', 'print', '00', 'A', '2015年9月计算机二级考试《C语言》真题及解析', '4', 'C 语言中标识符由字母、下切线、数字组成，且开4 必须是字母或下划线。另外，关键字不能\r\n作为标识符。因为C语言中区分大小写，所以B)法顶中的“FOR”可以作为标识符来用。A)选项中含有非法字符\r\nk，所以选择A');
INSERT INTO `exercises` VALUES ('23', '苦有定义，double 45 22 int i=0.k= 18r 则不符合C语言规定的赋值语句是', 'i- (a+k)< (i+k);', 'i=a%11', 'a=a++，i++', 'i=! a', 'B', '2015年9月计算机二级考试《C语言》真题及解析', '2', '取模运算符“%”，二元运算符，具有左结合性，参与运算的量均为整型。选项肿中的变量是\r\ndouble实型，所以B)不符合规定。');
INSERT INTO `exercises` VALUES ('24', '下关干逻辑运算符两偶运算对象的叙述中正确的是', '可以是任意合法的表达式', '只能是整数0或非0整数', '可以是结构体类型的数据', '只能是整數0或1', 'A', '2015年9月计算机二级考试《C语言》真题及解析', '4', 'C语言的逻辑运算符比较特别，它的操作数设有明确的故据夫型，可以是任意合法的表达式，\r\n所以选择A');
INSERT INTO `exercises` VALUES ('25', '以下毅述中错误的是', '可以给指针变量赋一个整数作为地址值', '函数可以返回地址值', '改变角数形参的值，不会改变对应实参的值', '当在程序的开头包含头文件stdio.h时，可以给指针变量財NULL', 'A', '2015年9月计算机二级考试《C语言》真题及解析', '1', '不能行一个整故直接赋给指针变量作为地址，所w是错误的函数的返回值可以是地址，\r\n即指针。函数调用中形参值的变化不会传递给实梦');
INSERT INTO `exercises` VALUES ('26', '设已有定义，float x 则以下对指针支量p进行定义且财切值的语句中正确的是', 'int *p= (tloat)x:', 'tloat *p-bx', 'float D=&x;', 'f1oat *p= 1024', 'B', '2015年9月计算机二级考试《C语言》真题及解析', '2', '指计是用来存放地址的支量，用(类型名+指针变量名)的无式定义。赋值时应将其个变量地\r\n址即紅x 賕給指针交量，所以选择B');
INSERT INTO `exercises` VALUES ('27', '以下数组定义中错误的是', 'int x[2][3]= [1,2,3,4,5,6}:', 'int x[[3]= (0);', 'int x[][3]= [{1,2,3).[4,5,61];', 'int x[2][3]= [[1.2].[3,4].[5,6});', 'D', '2015年9月计算机二级考试《C语言》真题及解析', '3', '选项中x[2][3]定《的是一个两行三列的二维故组。而在给故组元素赋值时却赋成了三行，\r\n所以错误。');
INSERT INTO `exercises` VALUES ('28', '设有定义，charp[]=(\'1\'，\'2’，\'3\')，*q=p，以下不能计算出一个char型的操所占字节数的表达式是', 'sizeot(p)', 'sixeot(char)', 'sixeot(*q)', 'nixeot(p[0])', 'A', '2015年9月计算机二级考试《C语言》真题及解析', '4', '根据慧目中的定义可(30港sizo of(o)，计算的是数组=中所有t素所占用的子节的，配不是\r\nchar型数据所占字节数。');
INSERT INTO `exercises` VALUES ('29', '以下权述中正确的是', '在C语言中，预处理命令行都以”开头', '预处理命令行必须位于C源程序的起始位置', 'Binc1ude <stdio.b>必须放在C程序的开头', 'C语言的预处理不能实现宏定义和条件编译的功能', 'A', '2015年9月计算机二级考试《C语言》真题及解析', '1', '预处理命令是以“”号开头的命会它们不是C语言的可执行命令这些命令应该在函故之\r\n外书写，一般在源文件的景扩面书写，但不是必须在起始位置书写所以B) 0)错误。C)语言的预外理能够\r\n实现完定义和条件编译等功能，所以D)增误。');
INSERT INTO `exercises` VALUES ('30', '在C语言中，合法的长整型常数是', '0.123456', '4962717', '9L', '5.321e2', 'C', '2015年9月计算机二级考试《C语言》真题及解析', '3', '无解析');
INSERT INTO `exercises` VALUES ('31', '执行语句X=(a=10，b=a--.后，x、a、b的值依次为', '10,10,9', '10,9,9', '9,10,9', '10,9,10', 'D', '2017年9月计算机二级考试《C语言》考前猜想卷', '4', '无解析');
INSERT INTO `exercises` VALUES ('32', '以下选项中非法的表达式是', 'a+1=a+1', 'a=b==0', '(Char.(100+ 100.', '7<=X<60', 'A', '2017年9月计算机二级考试《C语言》考前必做题', '5', '无解析');
INSERT INTO `exercises` VALUES ('33', '在以下选项中，不正确的赋值语句是', 'i++ ;', 'n1=(n2=(n3=0..;', 'm+n=1;', 'i=k==j ;', 'C', '2015年3月计算机二级考试《C语言》真题及解析', '3', '无解析');
INSERT INTO `exercises` VALUES ('34', '若有说明，double*m,n; 则能通过sCanf语句正确给输入项读人数据的程序段是', '*m= &n;scanf(\"%lf*m);', 'm= (double* )mallo (1 0);sc anf(\"%6f\'m);', 'm = &n;scant(%l*,n);', 'm = &n;scanf(\"%le\"\'m);', 'D', '2014年9月计算机二级考试《C语言》真题及解析', '3', '无解析');
INSERT INTO `exercises` VALUES ('35', '表示关系M<=N<=P的C语言表达式为', '(M<=N.AND(N<=P.', '(M<=N.&&(N<=P.', '(M<=N<=P.', '(M<=N.&(N<=P.', 'B', '第2章_C程序设计的初步知识', '4', '无解析');
INSERT INTO `exercises` VALUES ('36', '以下叙述中正确的是', '在scanf函数的格式串中，必须有与输入项一一对应的格式转换说明符', '只能在prinf函数中指定输入数据的宽度，而不能在scanf函数中指定输入数据占的宽度', 'scanf函数中的字符串，是提示程序员的，输入数据时不必管它', '复合语句也被称为语句块，它至少要包含两条语句', 'A', '第2章_C程序设计的初步知识', '2', '无解析');
INSERT INTO `exercises` VALUES ('37', '以下叙述中正确的是', '在scanf函数中的格式控制字符串是为了输入数据用的，不会输出到屏幕上', '在使用scanf函数输入整故或实故时，输入数据之间5 ?能用空格来分隔', '在Printf函数中，各个输出项只能是变量', '使用Printf函数无法输出百分号%', 'A', '第2章_C程序设计的初步知识', '3', '无解析');
...
-- Records of mistakes
-- ----------------------------
INSERT INTO `mistakes` VALUES ('15', '1', 'szl');
INSERT INTO `mistakes` VALUES ('16', '2', 'szl');
INSERT INTO `mistakes` VALUES ('17', '3', 'szl');
...
INSERT INTO `papers` VALUES ('29', 'null', 'null', null);
...
-- Records of users
-- ----------------------------
INSERT INTO `users` VALUES ('1', 'szl', '123456', 'user', null);
INSERT INTO `users` VALUES ('4', 'szl', '123456', 'admin', null);
